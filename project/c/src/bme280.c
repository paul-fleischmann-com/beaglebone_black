...
int bme280_init(bme280_dev_t *dev) {
    ...
    if(ioctl(dev->fd,I2C_SLAVE,BME280_ADDR) < 0)
    {
      return -2;
    } 
    uint8_t id[1]; 
    int read = i2c_read_reg(dev->fd,0xD0,id,1);
    /* return -3 if read was not successfull or successfull and value not 0x60 */
    if( (read < 0 ) ||  ( read >= 0 && id[0]!=0x60))
    {
      return -3;
    } 

    if  ( i2c_write_reg(dev->fd,0xE0,0xB6) < 0 );
    {
        return -4;
    }
    usleep(10000);
    if(read_calibration(dev)<0) 
    {
        return -5;
    }

    if  ( i2c_write_reg(dev->fd,0xF2,0x01) < 0 );
    {
        return -6;
    }

    if  ( i2c_write_reg(dev->fd,0xF4,0x57) < 0 );
    {
        return -7;
    }

    if  ( i2c_write_reg(dev->fd,0xF5,0xA0) < 0 );
    {
        return -8;
    }
    usleep(100000); 
